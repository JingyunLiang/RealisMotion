...
This repository is the official implementation of [RealisMotion: Decomposed Human Motion Control and Video Generation in the World Space](https://arxiv.org/abs/2508.08588).
...
><p align="center">
  <img width="800" src="https://github.com/JingyunLiang/RealisMotion/releases/download/v0.0/pipeline.gif">
</p>
...
<table>
  ...
  <tr>
    ...
    <td>
      <pre>
<code>huggingface-cli download --resume-download jingyunliang/RealisMotion --local-dir pretrained_models/RealisMotion</code>
      </pre>
    </td>
  </tr>
  <tr>
    ...
    <td>
      <pre>
<code>huggingface-cli download --resume-download theFoxofSky/RealisDance-DiT --local-dir pretrained_models/RealisDance-DiT</code>
      </pre>
    </td>
  </tr>
</table>
...
<code>CUDA_VISIBLE_DEVICES=0 python inference_t2v.py \
    --ref_path inputs/demo/internalaffairs/0_input_video.mp4 \
    --ref_cs_map_path inputs/demo/internalaffairs/cs_map.mp4 \
    --ref_mask_path inputs/demo/internalaffairs/mask.mp4 \
    --background_path inputs/demo/internalaffairs/0_input_video.mp4 \
    --background_mask_path inputs/demo/internalaffairs/mask.mp4 \
    --motion_folder inputs/demo/internalaffairs_falldown_internalaffairs \
    --ref_index 0 \
...
The human mask and hamer (hand pose) are optional, but providing them could improve the video quality. To obtain the human mask, one can install [MatAnyone](https://github.com/pq-yang/MatAnyone?tab=readme-ov-file#-installation) locally or use this [MatAnyone Online Demo](https://huggingface.co/spaces/PeiqingYang/MatAnyone). Without the human mask, we will extract one from the SMPL-X depth. To obtain the hamer, please refer to [Hamer Preparation](hmc/hamer/README.md). Without hamer, we will use the standard hand pose in SMPL-X.
